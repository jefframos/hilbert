...
Shader "Ciconia Studio/Effects/Ghost/Ghost Animated Details (World Projection)" {
    Properties {
        [Space(15)][Header(Main Properties)]
        [Space(10)]_Color ("Color", Color) = (1,1,1,1)
        _EmissiveIntensity ("Emissive Intensity", Range(0, 5)) = 0.25
        [Space(10)]_FresnelStrength ("Fresnel Strength", Range(0.05, 10)) = 0.5
        ...
        _NormalIntensityCloud ("Normal Intensity1", Range(0, 2)) = 1
        ...
        [Space(10)]_NormalIntensity2 ("Normal Intensity2", Range(0, 2)) = 0.5
        [MaterialToggle] _SwitchAnimationFlow2 ("Switch Animation Flow2", Float ) = 1.171035
    }
    SubShader {
        Tags {
            "Queue"="Transparent"
        ...
        Pass {
            ...
            float4 frag(VertexOutput i) : COLOR {
                ...
                float3 node_1815_nrm_base = lerp( node_782, (node_782*2.0+-1.0), _InvertEffect ) + float3(0,0,1);
                float3 node_1815_nrm_detail = lerp(float3(0,0,1),_BumpMap_var.rgb,_NormalIntensity) * float3(-1,-1,1);
                float3 node_1815_nrm_combined = node_1815_nrm_base*dot(node_1815_nrm_base, node_1815_nrm_detail)/node_1815_nrm_base.z - node_1815_nrm_detail;
                float3 node_1815 = node_1815_nrm_combined;
                float3 normalLocal = node_1815;
